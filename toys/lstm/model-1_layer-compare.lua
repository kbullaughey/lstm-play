  ...
  function trainer:train(dataset)
    ...
    while true do
      ...
      for t = 1,dataset:size() do
        ...
        batches = batches + 1
        --if t % 1000 == 0 then
          print(batches .. "| current partial error = " .. currentError / batches)
          print(batches .. "| current par sum: " .. par:sum())
          print(batches .. "| current gradPar sum: " .. gradPar:sum())
        --end
      ...
      print("currentLearningRate: " .. currentLearningRate)
...
function averageError(d, log)
  ...
  log = log or false
  for i=1,d:size() do
    local predictions = net:forward(d[i][1])
    err = err + criterion:forward(predictions, d[i][2])
    if log then
      criterion:backward(predictions, d[i][2])
      net:backward(d[i][1], criterion.gradInput)
      print("err[" .. i .. "]: " .. err)
...
net.ns.par:copy(torch.load('par'))
net.ns.gradPar:zero()
