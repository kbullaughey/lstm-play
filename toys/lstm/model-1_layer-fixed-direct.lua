...
cmd:option('-rate',0.001,'learn rate')
...
-- Add an input that is the position in the sequence
position = torch.repeatTensor(torch.range(1, maxLength):view(-1,1), N, 1, 1)
x = nn.JoinTable(3):forward({x, position})
...
chainModForward = lstm.MemoryChainDirect(2, {params.hidden}, maxLength)
chainModBackward = lstm.MemoryChainDirect(2, {params.hidden}, maxLength)
...
chainOut = nn.JoinTable(3)({chainOutForward, chainOutBackwardRereversed})
...
-- back.
chainOutReshaped = nn.Reshape(params.batch*maxLength, 2*params.hidden)(chainOut)
