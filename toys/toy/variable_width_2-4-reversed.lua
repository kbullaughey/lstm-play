...
inputs:cmul(mask)
reversedInputs = torch.Tensor():resizeAs(inputs):zero()

-- Iterate through the data, reversing the sequences
for i=1,n do
  local len = lengths[i]
  for t=1,len do
    reversedInputs[i][t] = inputs[i][len-t+1]
  end
end

-- Save the data as five columns, with the last two columns the lengths and
-- targets, in a t7 file.
data = nn.JoinTable(2):forward({reversedInputs,lengths:view(-1,1),target})
